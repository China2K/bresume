...
import com.bresume.core.common.lucene.config.IndexTypeOptions;
...
public class IndexUtils {
	...
	public static void rebuildOrUpdateIndex(ConfigBean config,Iterable<?extends BaseIndexBean> indexBeans,
...
//		ConfigBean config = ConfigurationLoader.getProductConf();
		...
		try {
			for (BaseIndexBean indexBean : indexBeans) {
				if (!create) {
					ConfigBean.Field key = config.getKey();
					writer.deleteDocuments(new Term(key.getName(), indexBean
							.getKey()));
				}
				writer.addDocument(createProductDoc(indexBean, config));
			}
		} catch (Exception e) {
	...
	private static Document createProductDoc(BaseIndexBean indexBean, ConfigBean config) {
		...
		doc.add(new Field(key.getName(), indexBean.getKey(), fieldType));
		for (ConfigBean.Field field : config.getFields()) {
			String value = getProperty(indexBean, field.getName());
